...
(* Equations? eval_or_match (es : list exp) (fuel : nat) (st : state nat) (env : sem_env val) *)
(*   : state nat * result (list val) val by wf (fuel, mutmeasure true es) lexicographic_rel := *)
(*   eval_or_match [] _ st _  => (st, Rval []); *)
(*   eval_or_match (e1::e2::es') fuel st env *)
(*     with eval_or_match fuel st env => { *)
(*     | (st', Rval vs) with eval_or_match (e2::es') fuel st' env => { *)
(*       | (st'', Rval vs'') with vs => { *)
(*         | [] => (st'', Rval (vs'')); *)
(*         | v::vs' => (st'', Rval (v::vs'')) *)
(*         }; *)
...
(*     | res => res *)
(*     }; *)
(*   eval_or_match [ERaise e'] fuel st env *)
(*     with eval_or_match [e'] fuel st env => { *)
(*     | (st', Rval (v::_)) => (st', Rerr (Rraise v)); *)
(*     | res => res *)
(*     }; *)

(*   eval_or_match [EHandle e' l] fuel st env with *)
(*     eval_or_match [e'] fuel st env => { *)
(*     | (st', Rerr (Rraise v)) => eval_or_pat l fuel st' env v bind_exn_v; *)
(*     | res => res *)
(*     }; *)

(*   eval_or_match [ELit l] _ st _ => (st, Rval [Litv l]); *)

(*   eval_or_match [EVar n] _ st env *)
(*     with nsLookup ident_string_beq n (sev env) => { *)
(*     | Some v' => (st, Rval [v']); *)
(*     | None => (st, Rerr (Rabort Rtype_error)) *)
(*     }; *)

(*   eval_or_match [ECon cn es'] fuel st env *)
(*     with do_con_check (sec env) cn (length es') => { *)
(*       | true with eval_or_match (rev es') fuel st env => { *)
(*         | (st', Rval vs) *)
...
(*   eval_or_match [EFun x e] _ st _ => (st, Rval [Closure env x e]); *)

(*   eval_or_match [EApp op es] 0 st env => (st, Rerr (Rabort Rtimeout_error)); *)

(*   eval_or_match [EApp op es] (S fuel') st env with *)
(*     eval_or_match (rev es) fuel' st env => { *)
(*     | (st', Rval vs) *)
(*       with op_beq op Opapp => { *)
(*       | true with do_opapp (rev vs) => { *)
(*         | Some (env', e) => eval_or_match [e] fuel' st' env'; *)
(*         | None => (st', Rerr (Rabort Rtype_error)) *)
(*         }; *)
...
(*   eval_or_match [ELog And e1 e2] fuel st env *)
(*     with eval_or_match [e1] fuel st env => { *)
(*     | (st', Rval (v::_)) => if val_beq v (Boolv true) *)
(*                           then eval_or_match [e2] fuel st env *)
...
(*   eval_or_match [ELog Or e1 e2] fuel st env *)
(*     with eval_or_match [e1] fuel st env => { *)
(*     | (st', Rval (v::_)) => if val_beq v (Boolv true) *)
(*                            then (st', Rval [v]) *)
(*                            else if val_beq v (Boolv false) *)
...
(*   eval_or_match [EIf c t e] fuel st env *)
(*     with eval_or_match [c] fuel st env => { *)
(*     | (st', Rval [v]) => if val_beq v ConvTrue *)
(*                         then eval_or_match [t] fuel st' env *)
(*                         else if val_beq v ConvFalse *)
(*                              then eval_or_match [e] fuel st' env *)
(*                              else (st', Rerr (Rabort Rtype_error)) *)
(*     | (st', Rval _) => (st', Rerr (Rabort Rtype_error)) *)
(*     | res => res *)
(*     }; *)

(*   eval_or_match [EMat e pes] fuel st env *)
(*     with eval_or_match [e] fuel st => { *)
(*     | (st', Rval (v'::vs')) => *)
(*         eval_or_pat pes fuel st' env v' bind_exn_v *)
(*     | res => res *)
(*     }; *)

(*   eval_or_match [ELet o e1 e2] fuel st env *)
(*     with eval_or_match [e1] fuel st env  => { *)
(*     | (st', Rval [v]) => *)
(*         eval_or_match [e2] fuel st' (update_sev env (nsOptBind o v (sev env))); *)
(*     | (st', Rval _) => (st', Rerr (Rabort Rtype_error)); *)
(*     | res => res *)
(*     }; *)

(*   eval_or_match [ELetrec funs e] fuel st env => *)
(*     if nodup_str (map (fun '(x,y,z) => x) funs) *)
(*     then eval_or_match [e] fuel st (update_sev env (build_rec_env funs env (sev env))) *)
(*     else (st, Rerr (Rabort Rtype_error)); *)

(*   eval_or_match [ETannot e t] fuel st env => eval_or_match [e] fuel st env; *)

(*   eval_or_match [ELannot e l] fuel st env => eval_or_match [e] fuel st env; *)

(* where eval_or_pat (es : list (pat * exp)) *)
(*           (fuel : nat) (st : state nat) (env : sem_env val) (match_v : val) (err_v : val) *)
(*   : state nat * result (list val) val by wf (fuel, mutmeasure false es) lexicographic_rel := *)

(*   eval_or_pat [] _ st _ _ err_v => (st, Rerr (Rraise err_v)); *)

(*   eval_or_pat ((p,e)::pes') fuel st env matched_v err_v => *)
(*     if nodup_str (pat_bindings p []) *)
(*     then (match pmatch (sec env) (refs st) p matched_v [] with *)
(*           | Match env_v' => *)
(*             eval_or_match [e] fuel st *)
(*                      {| sev := nsAppend (alist_to_ns env_v') (sev env); *)
(*                         sec := (sec env) |} *)
(*           | No_match => eval_or_pat pes' fuel st env matched_v err_v *)
(*           | Match_type_error => (st, Rerr (Rabort Rtype_error)) *)
(*           end) *)
(*     else (st, Rerr (Rabort Rtype_error)). *)
